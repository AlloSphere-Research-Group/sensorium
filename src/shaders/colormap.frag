...
void main() {
  ...
  //   0.6 + atan(pixel.r / 300)); hsv = vec3(0.55 + log(pixel.r / (90./255.) +
  //   1), 0.65 + pixel.r / (60./255.), 0.6 + atan(pixel.r / (300./255.)));

  // }else if(mapFunction == 1){ // nutrient
  //   // data_color = HSV(0.3 - log(pixel.r / 60. + 1), 0.9 + pixel.r / 90, 0.9
  //   + pixel.r / 90); hsv = vec3(0.3 + log(pixel.r / (60./255.) + 1), 0.9 +
  //   pixel.r / (90./255.), 0.9 + pixel.r / (90./255.) );
  ...
  if (mapFunction == 4) { // fishing
    ...
    // }
  } else if (mapFunction == 5) { // shipping
    //   // data_color = HSV(1 - log(pixel.r / 30. + 1), 0.6 + pixel.r / 100,
    //   0.6 + pixel.r / 60);
    hsv = vec3(1. - log(pixel.r / (30. / 255.) + 1),
               0.6 + pixel.r / (100. / 255.), 0.6 + pixel.r / (60. / 255.));
    alpha = pixel.r + hsv.b;
    alpha *= alpha / 4;
  } else if (mapFunction == 6) { // acidification
    //   // data_color = HSV(0.7 - 0.6 * log(pixel.r / 100. + 1), 0.5 +
    //   log(pixel.r / 100. + 1), 1);
    hsv = vec3(0.7 - 0.6 * log(pixel.r / (100. / 255.) + 1),
               0.5 + log(pixel.r / (100. / 255.) + 1.), 1.);
    alpha = pixel.r + hsv.g;
    alpha *= alpha * alpha;
  } else if (mapFunction == 7) { // sea level rise
    // data_color = HSV(0.6 + 0.2 * log(pixel.r / 100. + 1), 0.6 + log(pixel.r
    // / 60. + 1), 0.6 + log(pixel.r / 60. + 1));
    hsv = vec3(0.65 + 0.3 * log(pixel.r / (100. / 255.) + 1),
               0.6 + log(pixel.r / (60. / 255.) + 1.),
               0.6 + log(pixel.r / (60. / 255.) + 1.));
    alpha = pixel.r * 5;
    alpha *= alpha;
  }
  ...
  if (mapFunction == 4) {
    vec3 c = hsv2rgb(hsv);
    fragColor = vec4(c, alpha) * blend;
  } else {
    fragColor = vec4(pixel.rgb * blend, pixel.a);
  }
}